...
\item[Sitemap]\mbox{~}\\*
Im Sitemap-Bereich des Backends werden alle im System verwalteten Seiten aufgelistet. Zusätzlich können diese dort bearbeitet und verschoben werden.
...
Portlets verfügen über ein Template, welches beschreibt, wie ausgewählte Content Blocks dargestellt werden sollen. Innerhalb der Internetseite wird das erstellte Portlet positioniert und stellt dann die entsprechenden Inhaltselemente mit der im Portletlayout definierten Formatierung dar.
\item[Sitemap]\mbox{~}\\*
\end{description}

\subsection{Erweiterungen}
Die Funktionalität von Browser CMS kann durch das Hinzufügen von Content Blocks und Portlets erweitert werden.
...
Das komplette Backend von Browsercms wird mit Hilfe von HTML-Views aus Rails gerendert. JavaScript kommt lediglich bei der Darstellung verschiedener Feldtypen (z.B. Datumsdialog und Auswahlbox) innerhalb der Contentblöcke und beim Anlegen von Textinhalt durch die Verwendung des JavaScript-WYSIWYG-Editors FCKEditor zur Anwendung.

Die Erstellung der Contentblöcke und Portlets wird durch einen in Rails umgesetzten Generatorskript ermöglicht und vereinfacht somit die Erstellung für den Entwickler.
